...
void Poscar::restorePositions()
{
	convertToDirect();

	for (int i = 0; i < atomCoords.size(); i++)
	{
		//Larger than supercell:
		while(atomCoords[i].a > 1.0)
			atomCoords[i].a--;
		while (atomCoords[i].b > 1.0)
			atomCoords[i].b--;
		while (atomCoords[i].c > 1.0)
			atomCoords[i].c--;
		
		//Smaller than supercell (negative values)
		while(atomCoords[i].a < 0.0)
			atomCoords[i].a++;
		while (atomCoords[i].b < 0.0)
			atomCoords[i].b++;	
		while (atomCoords[i].c < 0.0)
			atomCoords[i].c++;
	}
}
...
std::vector <Coords> Poscar::allPeriodicImages(std::string instructions, int nImages=1)
{
...
convertToCartesian(fin);
	...
	if (instructions[0] == 's' || instructions[0] == 'S') ///keep periodic images close to original bonds case
	{
		...
		for (int i = 0; i < fin.size(); i++)
		{
			...
			for (int j = 0; j < atomCoords.size(); j++)
			{
				double distance = dist(fin[i], atomCoords[j]); ///not sure if this makes the code run facter, but in hopes that it does, setting the dist to a variable so the compiler dosent run the code multiple times
				if (distance < 0.1)
					copied = true;
			...
			if (!copied && closeToEdge)
				periodicImages.push_back(fin[i]);
		}
	}
	else ///keep all periodic images case
	{
		...
		for (int i = 0; i < fin.size(); i++)
		{
			bool copied = false;///assume there is a copy of extended atoms in original
			for (int j = 0; j < atomCoords.size(); j++)
				if (dist(fin[i], atomCoords[j]) < 0.1)
					copied = true;
	
			//If the atom is not a copy, keep it
			if (!copied)
				periodicImages.push_back(fin[i]);
		}
	}
...
std::vector <Coords> Poscar::allPeriodicImages(double sliceDist)
{
	std::vector <Coords> periodicImages;
...
void Poscar::fetchAtomPairs(double pairDist=-1.0, int nImages=1)
{
	//Failsafe
	if (pairDist < 0.0)
		pairDist = GENERAL_BOND_DISTANCE;

	convertToCartesian();

	std::vector <Coords> periodicImages = allPeriodicImages("nhave", nImages);
	...
	for (int i = 0; i < atomCoords.size(); i++)
		for (int j = 0; j < atomCoords.size(); j++)
			if (i != j)
			{
				...
				if (distance < pairDist && distance > 0.1)
	...
	for (int i = 0; i < periodicImages.size(); i++)
		for (int j = 0; j < atomCoords.size(); j++)
		{
			...
			if (distance_ < pairDist && distance_ > 0.1)
