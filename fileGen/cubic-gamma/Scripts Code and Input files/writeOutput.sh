...
echo "dir,workDir,nrg,nConv,irredKpts,aV,bV,cV,alpha,beta,gamma,vol,bg" >$origLoc/output.csv
...
#Get number of atoms
set nAg=`cat file__ | sed -n '7p' | awk '{print $1}'`
set nBi=`cat file__ | sed -n '7p' | awk '{print $2}'`
set nI=`cat file__ | sed -n '7p' | awk '{print $3}'`

#Gamma point band gap
cp $origLoc/gammaBandgap.py .
./gammaBandgap.py
set band=`grep BANDGAP bandgapOut | awk '{print $2}'`
rm gammaBandgap.py bandgapOut 

#Append all of this information to the csv line
echo "`pwd`,$wd,$nrg,$nConv,$ikp,$xV,$yV,$zV,$al,$be,$ga,$vol,$band" >>$origLoc/output.csv
