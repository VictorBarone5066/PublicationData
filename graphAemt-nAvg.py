...
for ke in pDat.keys():
    ...
    pdiffs[kd] = [data*0.2 for data in pDat[ke][WHICH]["mass"]]
for ke in pDatHi.keys():
    kd = FloatFormat(abs(ke - 1.720072), 2)
    for i in range(0, len(pdiffs[kd])):
        pdiffs[kd][i] += 0.8*pDatHi[ke][WHICH]["mass"][i]
...
if(WHICH == "cond"):
    ...
    ax[1].set_ylim(4.5, 13)
    ax[1].set_yticks([5.0, 6.5, 8.0, 9.5, 11.0, 12.5])
    pass
if(WHICH == "doss"):
...
fig.subplots_adjust(bottom=0.07, top=0.99, left=0.19, right=0.96, hspace=0.045, wspace=0.0)
